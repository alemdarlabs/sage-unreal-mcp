...
flowchart TB
  ...
  %% Şu an
  M25 ==> NOW(["ŞİMDİ BURADA<br/>━━━━━━━━━━━━━━━<br/>76 commit · 191 MCP tool · index_slot 30s→4s (7.5x)<br/>Phase 1+2+3+4.0/4.1/4.2(r1+r2)/4.3/4.4/4.5-r1+r2-b1..b9/4.6(r1+r2+r3-b1..b5)/4.7 CANLI<br/>SageTest UE 5.7 — 8K asset · 16K dep · 8337 class · 8336 inherit · 492 engine module<br/>Phase 4.5-r2-b9: import_static_mesh + import_skeletal_mesh + import_animation<br/>UE-MCP 448 action audit → Phase 4 yol haritası %80 done"]):::now
  ...
  subgraph P4["Phase 4 — Full UE-MCP capability parity"]
    ...
    P45["4.5 Asset advanced r1+r2-b1..b9 ✓ (31 tool)<br/>r1: mesh_bounds · mesh_collision · diagnose_registry<br/>list_redirectors · bulk_rename · move_folder · fixup_redirectors<br/>r2-b1: list · search · read_properties<br/>r2-b2: list/add/remove_socket — Static + Skeletal mesh<br/>r2-b3: list_textures · get_texture_info · set_texture_settings<br/>r2-b4: create_data_asset · delete_batch · reload_package<br/>r2-b5: list/set_mesh_material · set_sk_material_slots<br/>r2-b6: read/create/reimport_datatable<br/>r2-b7: import_texture · reimport<br/>r2-b8: export (auto-router)<br/>r2-b9: import_static_mesh · import_skeletal_mesh · import_animation<br/><i>r2-b10+: FTS · pivot · set_mesh_nav</i>"]:::done
