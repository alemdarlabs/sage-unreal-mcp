...
Bu projede `.Codex/agents/` altında 13 uzman agent tanımlıdır. **Her konuşmada konuya göre ilgili agent otomatik çağrılmalıdır.** Kullanıcı bir soru sorduğunda veya bir konu hakkında konuştuğunda, o konunun uzmanı olan agent'ı `/agent-adi` skill olarak çağır ve o uzmanlıkla yanıt ver.
...
**Status (2026-04-30): production code aktif** — Phase 1+2+3+4 + Milestone 1.5b tamamlandı, ilk gerçek dogfooding turu (11 gap fix tek oturumda) bitti, Mac→Windows production transition başlıyor. Mimari kararlar [`.Codex/decisions/`](.Codex/decisions/) (18 ADR), sistem dokümantasyonu [`.Codex/docs/`](.Codex/docs/), kabul edilen davranış kuralları [`.Codex/notes/lessons.md`](.Codex/notes/lessons.md).
...
- **Dağıtım modeli iki katmanlı**:
  - *Binary*: npm public — `npm install -g @alemdarlabs/sage-mcp` herkese açık (Codex/Codex pattern). Postinstall GitHub Releases'tan native binary indirir.
...
Detay: [`.Codex/docs/tech-stack.md`](.Codex/docs/tech-stack.md)
...
Detay: [`.Codex/docs/project-structure.md`](.Codex/docs/project-structure.md)
...
- **Dokümantasyon**:
  - [`.Codex/notes/diagram.md`](.Codex/notes/diagram.md) - milestone flow, tool distribution, retired graph context
  - [`.Codex/notes/ue-mcp-integration-plan.md`](.Codex/notes/ue-mcp-integration-plan.md) — UE-MCP 562 action'a karşı Sage yol haritası
  - [`.Codex/notes/ue-mcp-tasks.md`](.Codex/notes/ue-mcp-tasks.md) — per-tool task listesi
  - [`.Codex/notes/lessons.md`](.Codex/notes/lessons.md) — kabul edilen kurallar (en kritik: MVP scope-cut yasak, BP/Material GameThread marshal, nlohmann brace-init pitfall, MCP Streamable HTTP partial-impl, BridgeServer routing TODO trap, DRY middleware injection, C++ proje plugin install Source+Binaries)
  - [`.Codex/decisions/`](.Codex/decisions/) — 18 ADR (son: ADR-018 remove-kuzudb-graph-layer)
- **Yeni session devraldığında ilk bakılacak**: bu dosya → auto-memory (`sage_current_state.md` + `project_windows_transition.md` + `feedback_real_projects_caution.md`) → `.Codex/notes/diagram.md` → `.Codex/decisions/adr-017-multi-editor-routing-impl.md` → son commit `git log --oneline -12`.
...
Mac'te ikinci Codex session'ı (Kale projesi) gerçek MCP-client testi yaptı; 11 gerçek gap raporu, hepsi tek oturumda fix edildi:
...
**Pattern (yeni dogfooding'lerde tekrar)**: Test eden Codex `Gap #N` formatıyla rapor — *Hedef* + *Denenen tool(lar)* + *Args* + *Sonuç/hata* + *Eksik* + *Öneri (A/B/C öncelikli)* + *Workaround*. Geliştirici Codex fix + commit + bildirim. Bu format çok değerliydi, korunmalı.
...
- After ANY correction from the user: update `.Codex/notes/lessons.md` with the pattern
...
1. **Plan First**: Write plan to `.Codex/notes/todo.md` with checkable items
2. **Verify Plan**: Check in before starting implementation
3. **Track Progress**: Mark items complete as you go
4. **Explain Changes**: High-level summary at each step
5. **Document Results**: Add review section to `.Codex/notes/todo.md`
6. **Capture Lessons**: Update `.Codex/notes/lessons.md` after corrections
...
- **Token Discipline**: Every tool response respects the optimization principles in [`.Codex/docs/api-spec.md`](.Codex/docs/api-spec.md). No bloat.

## Documentation

- [Architecture](.Codex/docs/architecture.md) — Sistem mimarisi (topology, lifecycle, multi-editor)
- [Tech Stack](.Codex/docs/tech-stack.md) — Teknoloji kararları ve gerekçeleri
- [API Spec](.Codex/docs/api-spec.md) — MCP tool catalog, transport protocols, token optimization
- [Database Schema](.Codex/docs/database-schema.md) — active SQLite/audit storage + retired KuzuDB note
- [Project Structure](.Codex/docs/project-structure.md) — Klasör yapısı ve konvansiyonlar
- [Knowledge Graph](.Codex/docs/knowledge-graph.md) - retired KuzuDB graph layer and current inspection model
- [Transactions](.Codex/docs/transactions.md) — Transaction layer detayı
- [Compile Coordination](.Codex/docs/compile-coordination.md) — Live Coding vs full restart
- [MVP Roadmap](.Codex/docs/mvp-roadmap.md) — Phase 1 + Phase 2 milestone breakdown, risk register
- [Decisions (ADR)](.Codex/decisions/) — Architectural Decision Records (18 ADRs · son: ADR-018 remove-kuzudb-graph-layer)

## Kritik Dersler Özeti (lessons.md'den)

Tam liste [`.Codex/notes/lessons.md`](.Codex/notes/lessons.md) — schema/transport/build/UE-API touch eden değişiklikten önce zorunlu okuma. En kritik 8 madde:
...
5. **MCP Streamable HTTP partial-impl (paketleme blocker)** — sage-server şu an `Mcp-Session-Id` response header, GET `/mcp` SSE endpoint, OAuth metadata stub, `Mcp-Protocol-Version` header üretmiyor. Schema validation bittiğinde Codex stateless mode'da çalışıyor ama paketleme öncesi tamamlanmalı.
...
1. **Bu dosya** (AGENTS.md) — full snapshot + ticari bağlam + production disiplini + Windows/Mac build + dogfooding turu çıktıları
2. **`.Codex/notes/lessons.md`** — kabul edilmiş kurallar (yukarıdaki 8 madde + onlarca daha)
3. **`.Codex/decisions/adr-017-multi-editor-routing-impl.md`** — son büyük mimari karar
4. **`.Codex/decisions/adr-016-distribution-channel-npm.md`** — dağıtım stratejisi (binary kanalı)
...
- Test eden Codex `Gap #N` formatında raporluyorsa → fix + commit + bildirim pattern'i
